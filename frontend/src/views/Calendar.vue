  ...
  <div class="cal-app">
      ...
      <div class="month-grid">
        <div
          ...
          :class="{
            'm-other': !d.inMonth,
            'm-today': d.isToday,
            'm-sel': selDay && selDay.d === d.d && selDay.m === d.m,
          }"
        ...
        >
          ...
          <div class="m-chips">
            <div
              v-for="ev in d.evs.slice(0, 2)"
              :key="ev.id"
              class="m-chip"
              :class="colorClass(ev.event_type)"
            >
              {{ ev.title.slice(0, 4) }}{{ ev.title.length > 4 ? '…' : '' }}
            </div>
          ...
          <div class="w-slots">
            <template v-for="h in 24" :key="h">
              <div class="w-slot" :class="{ 'w-has': d.evsByHour[h - 1] }">
                ...
                <div
                  v-if="d.evsByHour[h - 1]"
                  class="w-ev"
                  :class="colorClass(d.evsByHour[h - 1][0].event_type)"
                >
    ...
    <div v-if="viewMode === 'month' && selDay" class="detail-panel">
      ...
      <div v-else class="event-cards">
        <div
          v-for="ev in selEvents"
          :key="ev.id"
          class="ev-card"
          :class="{ 'ev-done': ev.completed }"
        >
          ...
          <div class="ev-body">
            <div class="ev-r1">
              ...
              <span
                v-if="ev.recurrence && ev.recurrence !== 'none'"
                class="dv-rec"
                :title="'重复:' + ev.recurrence"
                >🔄</span
              >
              <span class="ev-type" :class="colorClass(ev.event_type)">{{ ev.event_type }}</span>
            </div>
            <div class="ev-r2">
              🕐 {{ formatTime(ev.event_time) }} · {{ relTime(ev.event_time) }}
            </div>
            <div v-if="ev.description" class="ev-r3">{{ ev.description }}</div>
          </div>
          <div class="ev-acts">
            <button v-if="!ev.completed" class="ea-btn ok" @click.stop="handleComplete(ev)">
              ○
            </button>
    ...
    <div v-if="error" class="err">
      {{ error }}<button class="err-x" @click="error = ''">×</button>
    </div>
  </div>
</template>
...
export default {
  ...
  setup() {
    ...
    const monthDays = computed(() => {
      ...
      for (let i = firstDOW - 1; i >= 0; i--) {
        ...
        arr.push({
          ...
          inMonth: false,
          isToday: false,
          evs: evOnDate(new Date(m === 0 ? y - 1 : y, m - 1, d)),
        })
      }
      ...
      for (let d = 1; d <= rem; d++) {
        arr.push({
          ...
          inMonth: false,
          isToday: false,
          evs: evOnDate(new Date(m === 11 ? y + 1 : y, m + 1, d)),
        })
      }
      return arr
    })
    ...
    const dayEvents = computed(() => {
      const d = currentDate.value
      return evOnDate(new Date(d.getFullYear(), d.getMonth(), d.getDate())).sort(
